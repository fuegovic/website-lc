...
- API is strict with unrecognized parameters and errors are not descriptive (usually "no body")
    ...
    - The use of [`dropParams`](./custom_config.md#dropparams)//TODO to drop "user", "frequency_penalty", "presence_penalty" params is required.
    - `stop` is no longer included as a default parameter, so there is no longer a need to include it in [`dropParams`](./custom_config.md#dropparams),//TODO unless you would like to completely prevent users from configuring this field.
